...
public class CircleTest {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            else if (list[0].compareTo(list[i]) == 0)
        ...
        for (int i = 0; i < list2.length; i++) {
            if (list2[0].compareTo(list2[i]) < 0)
                System.out.println(list2[0] + " has smaller size than " + list2[i]);
            else if (list2[0].compareTo(list2[i]) == 0)
                System.out.println(list2[0] + " has the same size as " + list2[i]);
            else if (list2[0].compareTo(list2[i]) > 0)
                System.out.println(list2[0] + " has the larger size than " + list2[i]);
        }
    }
}
